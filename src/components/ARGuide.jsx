...
import { Camera, Volume2, Info, X, MapPin, Loader2, AlertCircle, CheckCircle, Home } from 'lucide-react';
    ...
    <div className="w-full h-screen bg-gray-900 relative overflow-hidden">
      <div className="absolute top-0 left-0 right-0 z-20 bg-gradient-to-b from-black/70 to-transparent p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-white text-xl font-bold">AR Monument Guide</h1>
          </div>
          <button
            onClick={() => navigate('/')}
          ...
          <>
            ...
            {recognizedPlace && recognizedPlace.markers && (
              <div className="absolute inset-0 pointer-events-none">
                {recognizedPlace.markers.map(marker => (
                  ...
                  >
                    <div className="bg-emerald-500 w-4 h-4 rounded-full animate-ping absolute"></div>
                    <div className="bg-emerald-600 w-4 h-4 rounded-full"></div>
                    <div className="bg-black/90 text-white text-xs px-3 py-1.5 rounded-lg mt-2 whitespace-nowrap font-medium shadow-lg">
                      {marker.label}
                    </div>
                  </div>
                ))}
            ...
            {recognizedPlace && (
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/90 to-transparent p-6 pb-8">
                ...
                <div className="flex items-center gap-3">
                  <div className={`flex items-center gap-2 ${isSpeaking ? 'text-emerald-400' : 'text-gray-300'}`}>
                    <Volume2 className={`w-5 h-5 ${isSpeaking ? 'animate-pulse' : ''}`} />
                    <span className="text-sm font-medium">{isSpeaking ? 'Speaking...' : 'Audio guide ready'}</span>
                  </div>
                  {isSpeaking && (
                    <button
            ...
            {!recognizedPlace && (
              <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                <div className="w-48 h-48 border-4 border-emerald-500 rounded-lg animate-pulse"></div>
              </div>
            )}
      ...
      {showInfo && recognizedPlace && (
        <div className="absolute inset-0 bg-black/90 z-40 flex items-center justify-center p-4">
          <div className="bg-gray-800 rounded-xl max-w-lg w-full max-h-[80vh] overflow-y-auto shadow-2xl">
            ...
            <div className="p-4">
              <div className="mb-4">
                ...
                <p className="text-gray-200 text-sm leading-relaxed">{recognizedPlace.description}</p>
              </div>
              ...
              <button
                ...
                className="w-full mt-4 bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-700 hover:to-teal-800 text-white py-3 rounded-lg font-semibold flex items-center justify-center gap-2 transition"
              >
